...
const BiometricId = (props) => {
	...
	const handleSubmit = async () => {
		...
		if (selectedFile) {
			...
			formData.append("biometricID", selectedFile);
				...
				if (response.ok) {
					...
					try {
						...
						if (response.ok) {
							...
							setUpdating(false);
							toast.success("Biometric ID uploaded successfully!");
		...
		<div>
			...
			<Form>
				<Form.Group className="mb-3" controlId="IdCard1File">
					<Form.Label className="labelsStyle">Biometric ID</Form.Label>
					<Form.Control
