...
const UsageProfile = (userData) => {
	...
	const [formInputs, setFormInputs] = useState({
		awareness: usage_data.awareness_level || "",
		favoriteProduct: usage_data.usage_rate || "",
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await fetch(
				"https://100097.pythonanywhere.com/usage_form",
				{
					method: "POST",
					headers: {
			...
			if (response.ok) {
				...
				const updatedUser = {
					...currentstate,
					usage: {
						awareness_level: formInputs.awareness,
						usage_rate: formInputs.favoriteProduct,
						purpose: formInputs.purpose,
						others: formInputs.others,
					},
				};
			...
			<Form>
				<Form.Group className="mb-3" controlId="awareness">
					<Form.Label className="labelsStyle">
						Usage rate for your favourite product or service
					</Form.Label>
					<Form.Select
						aria-label="awareness"
						className="inputStyle"
						onChange={handleOnChange}
						value={formInputs.awareness}
					>
				...
				<Form.Group className="mb-3" controlId="favoriteProduct">
					<Form.Label className="labelsStyle">
						Awareness level while using the product or service
					</Form.Label>
					<Form.Select
						aria-label="favoriteProduct"
						className="inputStyle"
						onChange={handleOnChange}
						value={formInputs.favoriteProduct}
					>
				...
				<Form.Group className="mb-3" controlId="purpose">
					<Form.Label className="labelsStyle">
						Purpose while using the product or service
					</Form.Label>
					<Form.Select
						aria-label="purpose"
						className="inputStyle"
				...
				<Button variant="dark" className="w-100" onClick={handleSubmit}>
					{loading ? "Updating..." : "Update Usage Profile"}
				</Button>
			</Form>
		</div>
	);
