...
const MyProfileForm = (userData) => {
	...
	const handleSubmitProfile = async (e) => {
		...
		myFormData.append(
			"phone_sms",
			formData.phone_otp ? parseInt(formData.phone_otp) : 0
		);
		setUpdating(true);
		try {
			const response = await fetch(
				"https://100014.pythonanywhere.com/api/profile_update/",
				{
					method: "POST",
					body: myFormData,
				}
						...
						<Row>
							<Col sm={6}>
											...
											>
												{countries.map((country) => (
													<option
														...
														value={country.phoneCode_country}
													>
														{country.phoneCode_country}
													</option>
												))}
											</FormSelect>
										</Col>
								...
								<Form.Group className="mb-3" controlId="phoneotp">
									...
									<Form.Control
										...
										type="number"
										onChange={handleInputChange}
										placeholder="Enter OTP from phone number"
									/>
								</Form.Group>
							...
							<Col sm={6}>
								...
								<Form.Group className="mb-3" controlId="nativeLanguage">
									...
									<Form.Select
										aria-label="native_language"
										value={formData.native_langauage}
										name="native_langauage"
										className="inputStyle"
										onChange={handleInputChange}
									>
